...
import { OrgNonprofitStaffListComponent, OrgSchoolStaffListComponent } from './org-school-staff-list.component';
...
@Component({
  selector: 'app-org-nonprofit-group',
  standalone: true,
  imports: [CommonModule, SimpleTabsetComponent, HcclOrganizationCrudComponent, HcclOrganizationListComponent,
    ...
    OrgNonprofitStaffListComponent],
  templateUrl: './org-nonprofit-group.component.html' 
})
export class OrgNonprofitGroupComponent extends AbstractEntityGroupComponent<HcclOrganizationCrudWrapper> implements OnInit {  
  ...
  organizationTypeCode: string = 'NONPROFIT';
  ...
  //  o.alertMessage = 'Modal to show catalog entry';
    ...
    o.getNavigateUrl = (entityId: string, baseRoute: string): any[] => {
      return ['/ecoadmin-dashboard/orgs/nonprofits', entityId, 'details'];
    };
    return o;
  }

  entityForCreate?: HcclOrganizationCrudWrapper ;
  showingNonprofitList = true;

  onClickAddNonprofit(): void {
  ...
  onOnboardCloseRefresh(): void {
    this.currentTabId = 'list';
    this.showingTabset = true;
    this.showingNonprofitList = false;
    setTimeout(() => {
      this.showingNonprofitList = true;
    });
  }

  protected override getBaseRoute(): string {
    return '/ecoadmin-dashboard/orgs/nonprofits';
  }
  ...
  onClickOrgStaffRowBehavior(): OnRowClickBehavior {
   ...
   // o.alertMessage = 'Modal to show catalog entry';
    ...
    //o.doNotNavigate = true;
    o.getNavigateUrl = (entityId: string, baseRoute: string): any[] => {
      return ['/ecoadmin-dashboard/orgs/nonprofits', this.id, 'staffmember', entityId];
    };
    return o;
  }
} 