...
type CitizenProfileWithConsents = StudentProfileUIGETData & {
  consents?: MultiConsentRequestGETData & { contracts?: ProfileConsentItem[] };
};

@Component({
  selector: 'app-citizen-profile-ui',
  standalone: true,
  imports: [
    CommonModule,
    FormsModule,
  ...
  templateUrl: './citizen-profile-ui.component.html',
  styleUrl: './citizen-profile-ui.component.scss',
})
export class CitizenProfileUiComponent implements OnInit, OnChanges {
  @Input() id = '';
  ...
  private citizenProfileUi: CitizenProfileWithConsents = {};
  ...
  get citizenProfile(): CitizenProfileWithConsents {
    return this.citizenProfileUi;
  }
  ...
  get consents(): ProfileConsentItem[] {
    return this.citizenProfileUi.consents?.contracts || [];
  }
  ...
  get inviteeProfileId(): string {
    return (
      (this.citizenProfileUi.student?.id || '').trim() ||
      (this.id || '').trim()
    );
  }
  ...
  onSavePersonalDetails(): void {
    ...
    this.hcclService.updateHcclPersonById(personId, payload).subscribe({
      next: () => {
        ...
        if (this.citizenProfileUi.student?.theUser) {
          this.citizenProfileUi.student.theUser.person = this.clonePerson(this.personDraft);
        }
  ...
  onUseAddressForMatchingChange(value: boolean): void {
    ...
    if (!this.citizenProfileUi.feedProfile) {
      this.citizenProfileUi.feedProfile = {};
    }
    if (!this.citizenProfileUi.feedProfile.feedInputs) {
      this.citizenProfileUi.feedProfile.feedInputs = {};
    }
    this.citizenProfileUi.feedProfile.feedInputs.usingLocation = this.useAddressForMatching;
  }

  onSaveFeedSettings(): void {
    const citizenProfileId = this.citizenProfileUi.student?.id || '';
    if (!citizenProfileId) {
      this.feedSaveError = 'Citizen profile id is required before saving feed settings.';
      this.feedSaveSuccess = '';
      return;
    }

    const feedProfile = this.citizenProfileUi.feedProfile || {};
    ...
    if (feedProfile.id) {
      const putData: CatalogEntryFeedProfilePUTData = {
        userProfileId: feedProfile.userProfileId || citizenProfileId,
    ...
    const postData: CatalogEntryFeedProfilePOSTData = {
      userProfileId: citizenProfileId,
      minScore: feedProfile.minScore ?? 0,
      maxDistanceInMiles: feedProfile.maxDistanceInMiles ?? 0,
      distanceInVocode: feedProfile.distanceInVocode ?? 0,
      feedProfileDataJson: feedProfile.feedProfileDataJson,
    ...
    this.hcclService.createCatalogEntryFeedProfile(postData).subscribe({
      next: (response: any) => {
        ...
        if (createdId) {
          this.citizenProfileUi.feedProfile.id = createdId;
        }
        this.citizenProfileUi.feedProfile.userProfileId = citizenProfileId;
        this.citizenProfileUi.feedProfile.feedInputs = feedInputs;
        this.feedSaveSuccess = 'Feed settings saved.';
      },
      error: (err) => {
        this.feedSaving = false;
  ...
  onAddressCreated(addressId: string): void {
    ...
    this.hcclService.updateHcclPersonById(personId, payload).subscribe({
      next: () => {
        ...
        this.citizenProfileUi.feedProfile.feedInputs.usingLocation = true;
        this.personSnapshot = this.clonePerson(this.personDraft);
        if (this.citizenProfileUi.student?.theUser) {
          this.citizenProfileUi.student.theUser.person = this.clonePerson(this.personDraft);
        }
      ...
      next: () => {
        ...
        this.hcclService.updateHcclPersonById(personId, payload).subscribe({
          next: () => {
            ...
            this.citizenProfileUi.feedProfile.feedInputs.usingLocation = false;
            this.personSnapshot = this.clonePerson(this.personDraft);
            if (this.citizenProfileUi.student?.theUser) {
              this.citizenProfileUi.student.theUser.person = this.clonePerson(this.personDraft);
            }
  ...
  private loadProfile(): void {
    ...
    this.hcclContextService.waitForReady$().subscribe({
      next: (context) => {
        ...
        if (!contextId) {
          this.loading = false;
          this.error = 'Citizen profile id is required.';
          this.citizenProfileUi = {};
          this.personDraft = {};
          this.personSnapshot = {};
          return;
        }
      ...
      error: () => {
        this.loading = false;
        this.error = 'Unable to resolve citizen context.';
      },
    });
  }
  ...
  private loadProfileById(resolvedId: string): void {
    ...
    this.hcclService.resolveStudentProfileData(resolvedId).subscribe({
      next: (data) => {
        this.loading = false;
        this.citizenProfileUi = (data || {}) as CitizenProfileWithConsents;
        this.activeConsent = null;
      ...
      error: (err) => {
        this.loading = false;
        this.error = err?.error?.message || err?.message || 'Unable to load citizen profile data.';
      },
    });
  }
