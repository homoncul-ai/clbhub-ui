...
@Component({
  ...
  imports: [CommonModule, CatalogEntryInterestListComponent, CommonModule, SimpleTabsetComponent,
    ...
    CatalogEntryInterestCrudComponent, StdBubaComponent, PersonalStatementSelectorComponent, CitizenEngageInterestComponent, FormsModule],
  template: `
  ...
  (selectionChange)="onPersonalStatementChange($event)">
      ...
      <div class="row" *ngIf="getSelectedInterestId() !== ''">
        <div class="col-12">
          <app-citizen-engage-interest 
            [interestId]="getSelectedInterestId()"
            (componentRequiresRefresh)="onChildComponentRefresh()"></app-citizen-engage-interest> 
        </div>
      </div>
    </div>
  `,
...
export class CitizenEngageComponent 
