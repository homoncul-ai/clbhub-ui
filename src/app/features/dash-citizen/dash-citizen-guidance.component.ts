...
@Component({
  selector: 'app-dash-citizen-guidance',
  standalone: true,
...
export class DashCitizenGuidanceComponent implements OnInit {
  ...
  constructor() {
    console.log('DashCitizenGuidanceComponent initialized');
  ...
  protected loadInfo() {
    ...
    this.hcclService.resolveGuidanceUIData().subscribe((data) => {
      console.log('Citizen Dashboard UI data loaded:', data);
  ...
  onClickTicket(ticket: WorkRequestGETData): void {
    ...
    this.router.navigate(['/citizen/guidance/workrequests', ticket.id,'ticket']);
  }
}
