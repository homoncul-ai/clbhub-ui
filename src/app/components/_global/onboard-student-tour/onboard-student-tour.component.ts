...
export class OnboardStudentTourComponent implements OnInit, OnDestroy {
  ...
  private maybeStartFromUrl(): void {
    ...
    // Ecoadmin (or any non-student shell) must land on the student dashboard
    // route so the main pane + tour targets match the walkthrough.
    if (!this.isOnStudentDashboard()) {
      const tree = this.router.parseUrl(this.router.url);
      void this.router.navigate([STUDENT_DASHBOARD_HOME], {
  ...
  private isOnStudentDashboard(): boolean {
    ...
    return path === STUDENT_DASHBOARD_HOME || path.startsWith('/student-dashboard/');
  }
  ...
  private scheduleStart(tourKey: string): void {
    ...
    if (!this.findMenuTarget('My Dashboard')) {
      ...
      this.menuService.beginTourPreview('student');
    }
