...
export class WorkRequestTypeRefGroupComponent extends AbstractEntityGroupComponent<WorkRequestTypeRefCrudWrapper> implements OnInit {  
  ...
  ngOnInit(): void {
    this.route.params.subscribe(params => {
      ...
      if (!id) {
        ...
        this.entity = WorkRequestTypeRefCrudWrapper.newInstanceForCreate(this.hcclService);
      } else {
      if (id) {
        this.id = id;
        ...
        this.loadEntityById(id).then(entity => {
          ...
          console.error('Error loading WorkRequestTypeRef:', error);
        });
      }
      }
    });
  ...
  protected async loadEntityById(id: string): Promise<WorkRequestTypeRefCrudWrapper> {
    const ref = await this.hcclService.getWorkRequestTypeRefById(id).toPromise();
    if (!ref) {
      throw new Error('WorkRequestTypeRef not found');
    }
    this.entity = new WorkRequestTypeRefCrudWrapper(ref, this.hcclService);
    return this.entity;
  }
  ...
  protected setupTabs(): SimpleTab[] {
    return [
      new SimpleTab('list', 'List', '', 
        () => {
          this.router.navigate(['/ecoadmin-dashboard/workrequesttyperefs']);
        },
        () => {
          return true;
        }
      ...
      new SimpleTab('details', 'Details', '', 
        () => {
          ...
          this.router.navigate(['/ecoadmin-dashboard/workrequesttyperefs', this.id, 'details']);
        },
        () => {
      ...
      new SimpleTab('debug', 'Debug', '', 
        () => {
          ...
          this.router.navigate(['/ecoadmin-dashboard/workrequesttyperefs', this.id, 'debug']);
        },
        () => {
          return true;
        }
      ...
      new SimpleTab('fk_menu', 'FK_MENU', '', 
        () => {
          ...
          this.router.navigate(['/ecoadmin-dashboard/workrequesttyperefs', this.id, 'fk_menu']);
        },
        () => {
          return true;
        }
