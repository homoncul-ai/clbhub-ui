...
// Generate the new pmessage-list.component.ts   files using this template
...
import { PMessageGETData, PMessageCriteria, PMessageGETDataSearchResults } from '../../../restsvc/hccl.service';
...
/**
 * Component for displaying and managing PMessage data using HcclService
 * Extends AbstractListComponent for common grid functionality
 */

@Component({
  selector: 'app-pmessage-list',
  standalone: true,
...
export class PMessageListComponent extends AbstractListComponent<PMessageGETData, PMessageCriteria, PMessageGETDataSearchResults> {
  ...
  protected getGridColumns(): any[] {
    return [
      ...
      { id: 'authorUserProfileStr', header: [{ text: 'Author User Profile', align: 'center' }, { content: 'inputFilter' }], minWidth: 150, adjust: true },
...
//      { id: 'createdByInfo', header: [{ text: 'Created By', align: 'center' }], minWidth: 120, adjust: true },
  ...
  protected createCriteria(): PMessageCriteria {
    return {
      pageNumber: 1,
      pageSize: 50,
      isPaging: true
  ...
  protected findEntities(criteria: PMessageCriteria): Observable<PMessageGETDataSearchResults> {
    return this.hcclService.findPMessages(criteria);
  }

  protected hasSearchResults(response: PMessageGETDataSearchResults): boolean {
    return !!response.searchResults;
  }

  protected getSearchResults(response: PMessageGETDataSearchResults): PMessageGETData[] {
    return response.searchResults || [];
  }
   ...
   * @returns 
   */
  protected override async formatEntityDataAsync(entity: PMessageGETData): Promise<any> {
    // Every attribute of the form [prefix]Id is an "foreign key" and should be replaced with the displaytext of the crudwrapper
    const authorUserProfileStr  :string = entity.authorUserProfileId  == null ? 'unknown' : 
       (await HcclUserProfileCrudWrapper.newInstance(entity.authorUserProfileId , this.hcclService)).getDisplayText();
      ...
      return {
        ...
        authorUserProfileStr: authorUserProfileStr
      };
    }
  }
