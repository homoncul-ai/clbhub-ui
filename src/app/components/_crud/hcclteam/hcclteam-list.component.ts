...
/**
 * Component for displaying and managing HcclTeam data using HcclService
 * Extends AbstractListComponent for common grid functionality
 */

@Component({
  selector: 'app-hcclteam-list',
  standalone: true,
...
export class HcclTeamListComponent extends AbstractListComponent<HcclTeamGETData, HcclTeamCriteria, HcclTeamGETDataSearchResults> {
  ...
  protected getGridColumns(): any[] {
    return [
      ...
      { id: 'teamTypeId', header: [{ text: 'Team Type ID', align: 'center' }, { content: 'inputFilter' }], minWidth: 120, adjust: true },
      { id: 'organizationId', header: [{ text: 'Organization ID', align: 'center' }, { content: 'inputFilter' }], minWidth: 120, adjust: true },
      { id: 'teamParentId', header: [{ text: 'Team Parent ID', align: 'center' }, { content: 'inputFilter' }], minWidth: 120, adjust: true },
      { id: 'teamParentEntityType', header: [{ text: 'Team Parent Entity Type', align: 'center' }, { content: 'inputFilter' }], minWidth: 150, adjust: true },
      { id: 'teamParentName', header: [{ text: 'Team Parent Name', align: 'center' }, { content: 'inputFilter' }], minWidth: 150, adjust: true },
...
//      { id: 'createdByInfo', header: [{ text: 'Created By', align: 'center' }], minWidth: 120, adjust: true },
  ...
  protected createCriteria(): HcclTeamCriteria {
    return {
      pageNumber: 1,
      pageSize: 50,
      isPaging: true
  ...
  protected findEntities(criteria: HcclTeamCriteria): Observable<HcclTeamGETDataSearchResults> {
    return this.hcclService.findHcclTeams(criteria);
  }

  protected hasSearchResults(response: HcclTeamGETDataSearchResults): boolean {
    return !!response.searchResults;
  }

  protected getSearchResults(response: HcclTeamGETDataSearchResults): HcclTeamGETData[] {
    return response.searchResults || [];
  }

  protected override formatEntityData(entity: HcclTeamGETData): any {
    return {
