...
import { ProviderRequestTypeRefGETData, ProviderRequestTypeRefCriteria, ProviderRequestTypeRefGETDataSearchResults } from '../../../restsvc/hccl.service';
import { AbstractListComponent } from '@app/components/_global/abstract-list/abstract-list.component';
import { Observable } from 'rxjs';

/**
 * Component for displaying and managing ProviderRequestTypeRef data using HcclService
 * Extends AbstractListComponent for common grid functionality
 */

@Component({
  selector: 'app-providerrequesttyperef-list',
  standalone: true,
...
export class ProviderRequestTypeRefListComponent extends AbstractListComponent<ProviderRequestTypeRefGETData, ProviderRequestTypeRefCriteria, ProviderRequestTypeRefGETDataSearchResults> {
  ...
  constructor(   
    ...
    this.searchHeading = 'Provider Request Type Refs';
...
//      { id: 'createdByInfo', header: [{ text: 'Created By', align: 'center' }], minWidth: 120, adjust: true },
  ...
  protected createCriteria(): ProviderRequestTypeRefCriteria {
    return {
      pageNumber: 1,
      pageSize: 50,
      isPaging: true
  ...
  protected findEntities(criteria: ProviderRequestTypeRefCriteria): Observable<ProviderRequestTypeRefGETDataSearchResults> {
    return this.hcclService.findProviderRequestTypeRefs(criteria);
  }

  protected hasSearchResults(response: ProviderRequestTypeRefGETDataSearchResults): boolean {
    return !!response.searchResults;
  }

  protected getSearchResults(response: ProviderRequestTypeRefGETDataSearchResults): ProviderRequestTypeRefGETData[] {
    return response.searchResults || [];
  }
   ...
   * @returns 
   */
  protected override async formatEntityDataAsync(entity: ProviderRequestTypeRefGETData): Promise<any> {
    // No FK attributes to format for ProviderRequestTypeRef
