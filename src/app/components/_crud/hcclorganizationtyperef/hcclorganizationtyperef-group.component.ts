...
export class HcclOrganizationTypeRefGroupComponent extends AbstractEntityGroupComponent<HcclOrganizationTypeRefCrudWrapper> implements OnInit {  
  ...
  ngOnInit(): void {
    this.route.params.subscribe(params => {
      ...
      if (!id) {
        ...
        this.entity = HcclOrganizationTypeRefCrudWrapper.newInstanceForCreate(this.hcclService);
      } else {
      if (id) {
        this.id = id;
        ...
        this.loadEntityById(id).then(entity => {
          ...
          console.error('Error loading HcclOrganizationTypeRef:', error);
        });
      }
      }
    });
  ...
  protected async loadEntityById(id: string): Promise<HcclOrganizationTypeRefCrudWrapper> {
    const ref = await this.hcclService.getHcclOrganizationTypeRefById(id).toPromise();
    if (!ref) {
      throw new Error('HcclOrganizationTypeRef not found');
    }
    this.entity = new HcclOrganizationTypeRefCrudWrapper(ref, this.hcclService);
    return this.entity;
  }
  ...
  protected setupTabs(): SimpleTab[] {
    return [
      new SimpleTab('list', 'List', '', 
        () => {
          this.router.navigate(['/ecoadmin-dashboard/hcclorganizationtyperefs']);
        },
        () => {
          return true;
        }
      ...
      new SimpleTab('details', 'Details', '', 
        () => {
          ...
          this.router.navigate(['/ecoadmin-dashboard/hcclorganizationtyperefs', this.id, 'details']);
        },
        () => {
      ...
      new SimpleTab('debug', 'Debug', '', 
        () => {
          ...
          this.router.navigate(['/ecoadmin-dashboard/hcclorganizationtyperefs', this.id, 'debug']);
        },
        () => {
          return true;
        }
      ...
      new SimpleTab('fk_menu', 'FK_MENU', '', 
        () => {
          ...
          this.router.navigate(['/ecoadmin-dashboard/hcclorganizationtyperefs', this.id, 'fk_menu']);
        },
        () => {
          return true;
        }
