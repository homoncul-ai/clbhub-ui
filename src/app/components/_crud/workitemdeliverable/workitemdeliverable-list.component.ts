...
import { WorkItemDeliverableGETData, WorkItemDeliverableCriteria, WorkItemDeliverableGETDataSearchResults } from '../../../restsvc/hccl.service';
import { AbstractListComponent } from '@app/components/_global/abstract-list/abstract-list.component';
import { Observable } from 'rxjs';

/**
 * Component for displaying and managing WorkItemDeliverable data using HcclService
 * Extends AbstractListComponent for common grid functionality
 */

@Component({
  selector: 'app-workitemdeliverable-list',
  standalone: true,
...
export class WorkItemDeliverableListComponent extends AbstractListComponent<WorkItemDeliverableGETData, WorkItemDeliverableCriteria, WorkItemDeliverableGETDataSearchResults> {
  ...
  constructor(   
    ...
    this.searchHeading = 'Work Item Deliverables';
  ...
  protected getGridColumns(): any[] {
    return [
      ...
      { id: 'delivTypeCode', header: [{ text: 'Delivery Type Code', align: 'center' }, { content: 'inputFilter' }], minWidth: 150, adjust: true },
...
//      { id: 'createdByInfo', header: [{ text: 'Created By', align: 'center' }], minWidth: 120, adjust: true },
  ...
  protected createCriteria(): WorkItemDeliverableCriteria {
    return {
      pageNumber: 1,
      pageSize: 50,
      isPaging: true
  ...
  protected findEntities(criteria: WorkItemDeliverableCriteria): Observable<WorkItemDeliverableGETDataSearchResults> {
    return this.hcclService.findWorkItemDeliverables(criteria);
  }

  protected hasSearchResults(response: WorkItemDeliverableGETDataSearchResults): boolean {
    return !!response.searchResults;
  }

  protected getSearchResults(response: WorkItemDeliverableGETDataSearchResults): WorkItemDeliverableGETData[] {
    return response.searchResults || [];
  }
   ...
   * @returns 
   */
  protected override async formatEntityDataAsync(entity: WorkItemDeliverableGETData): Promise<any> {
    // No FK components needed for WorkItemDeliverable based on the interface
